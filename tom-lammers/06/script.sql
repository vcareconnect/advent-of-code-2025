...
SET @RowCount =
  (SELECT COUNT(*)
   FROM
     (SELECT top (1) *
      FROM STRING_SPLIT(replace(@input, char(10), ''), char(13))) AS A CROSS APPLY STRING_SPLIT(value, ' ') AS CellInput
   WHERE CellInput.[value] <> '')
SELECT CONVERT(nvarchar(MAX), rowInput.value) AS RowValue,
       CONVERT(nvarchar(MAX), CellInput.value) AS CellInputValue,
       RowNumber,
       (ROW_NUMBER() OVER (PARTITION BY RowNumber
                           ORDER BY
                             (SELECT NULL)) -1) % @RowCount AS RowIndex INTO #Temp
FROM
  (SELECT value,
          ROW_NUMBER() OVER (
                             ORDER BY
                               (SELECT NULL)) AS RowNumber
   FROM STRING_SPLIT(replace(@input, char(10), ''), char(13))) AS rowInput CROSS APPLY STRING_SPLIT(CONVERT(nvarchar(MAX), value), ' ') AS CellInput
WHERE CellInput.[value] <> ''
ORDER BY RowNumber,
         RowIndex DECLARE @ValueLength int =
  (SELECT max(LEN(CellInputValue))
   FROM #Temp)
SELECT CASE Modifier.CellInputValue
           WHEN '+' THEN SUM(CONVERT(bigint, numbers.CellInputValue))
           WHEN '*' THEN ROUND(EXP(SUM(LOG(CONVERT(int, numbers.CellInputValue)))), 0)
       END AS value INTO #CalculationResults
FROM
  (SELECT CellInputValue,
          RowIndex
   FROM #Temp
   WHERE RowNumber = 5) AS Modifier
JOIN
  (SELECT CellInputValue,
          RowIndex
   FROM #Temp
   WHERE RowNumber <> 5) AS numbers ON Modifier.RowIndex = numbers.RowIndex
GROUP BY Modifier.CellInputValue,
         numbers.RowIndex
SELECT SUM(value) AS Answer1
FROM #CalculationResults -- 123 328   51  64
--  45  64  387  23
--   6  98  215 314
-- *   +   *   +
...
SELECT RowIndex,
       MAX(LEN(CONVERT (NVARCHAR, CellInputValue))) AS CellLength INTO #CellLengths
FROM #Temp
GROUP BY RowIndex
ORDER BY RowIndex
UPDATE #temp
SET CellInputValue = SUBSTRING(RowValue, ISNULL(
                                                  (SELECT sum(CellLength + 1)
                                                   FROM #CellLengths c
                                                   WHERE c.RowIndex < #CellLengths.RowIndex)+ 1, 0), CellLength + 1)
FROM #CellLengths
JOIN #temp ON #temp.RowIndex = #CellLengths.RowIndex DECLARE @spacer NVARCHAR(MAX) = '    ';


SELECT Modifier.CellInputValue,
       Modifier.RowIndex,
       trim(SUBSTRING((row1.CellInputValue), 1, 1) + SUBSTRING((row2.CellInputValue), 1, 1) + SUBSTRING((row3.CellInputValue), 1, 1) + SUBSTRING((row4.CellInputValue), 1, 1)) AS pivot1,
       trim(SUBSTRING((row1.CellInputValue), 2, 1) + SUBSTRING((row2.CellInputValue), 2, 1) + SUBSTRING((row3.CellInputValue), 2, 1) + SUBSTRING((row4.CellInputValue), 2, 1)) AS pivot2,
       trim(SUBSTRING((row1.CellInputValue), 3, 1) + SUBSTRING((row2.CellInputValue), 3, 1) + SUBSTRING((row3.CellInputValue), 3, 1) + SUBSTRING((row4.CellInputValue), 3, 1)) AS pivot3,
       trim(SUBSTRING((row1.CellInputValue), 4, 1) + SUBSTRING((row2.CellInputValue), 4, 1) + SUBSTRING((row3.CellInputValue), 4, 1) + SUBSTRING((row4.CellInputValue), 4, 1)) AS pivot4 INTO #CalculationResults2
FROM
  (SELECT CellInputValue,
          RowIndex,
          RowValue
...
JOIN
  (SELECT CellInputValue,
          ...
          RowValue
   FROM #Temp
   WHERE RowNumber = 1) AS row1 ON Modifier.RowIndex = row1.RowIndex
JOIN
  (SELECT CellInputValue,
          RowIndex,
          RowValue
   FROM #Temp
   WHERE RowNumber = 2) AS row2 ON Modifier.RowIndex = row2.RowIndex
JOIN
  (SELECT CellInputValue,
          RowIndex,
          RowValue
   FROM #Temp
   WHERE RowNumber = 3) AS row3 ON Modifier.RowIndex = row3.RowIndex
JOIN
  (SELECT CellInputValue,
          RowIndex,
          RowValue
   FROM #Temp
   WHERE RowNumber = 4) AS row4 ON Modifier.RowIndex = row4.RowIndex
ORDER BY RowIndex
SELECT SUM(CASE CellInputValue
               WHEN '+' THEN convert(bigint, pivot1)+ convert(bigint, pivot2) +convert(bigint, pivot3) +convert(bigint, pivot4)
               WHEN '*' THEN isnull(nullif(convert(bigint, trim(pivot1)), ''), 1)* isnull(nullif(convert(bigint, trim(pivot2)), ''), 1) *isnull(nullif(convert(bigint, trim(pivot3)), ''), 1) * isnull(nullif(convert(bigint, trim(pivot4)), ''), 1)
           END) AS Answer2
FROM #CalculationResults2
